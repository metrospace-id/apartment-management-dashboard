...
const Toggle = ({ label, disabled, ...props }: ToggleProps) => (
  <label
    className={`inline-flex items-center w-fit ${disabled ? 'opacity-70 cursor-not-allowed' : 'cursor-pointer'}`}
  >
    <div>
      <input
        type="checkbox"
    ...
    {label && (
      <span className="ms-3 text-sm font-medium text-slate-600 dark:text-white">
        {label}
      </span>
    )}
  </label>
