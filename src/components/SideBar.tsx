...
const SideBar = ({ open }: SideBarProps) => {
  ...
  return (
    <aside
      className={`${open ? 'ml-0' : '-ml-[250px]'} transition-all absolute z-30 w-[250px] px-4 bg-sky-900 shadow-sm dark:bg-black dark:border-slate-900 md:relative flex flex-col max-h-screen`}
    >
      <div className="m-auto p-4 text-center shadow-lg -mx-4">
        <img
          src="/images/metrospace-logo-white.png"
          alt="logo"
          className="m-auto w-[150px]"
        />
        <h1 className="text-xs text-white">
          {process.env.REACT_APP_APARTMENT_NAME}
        </h1>
      </div>
      ...
      <div className="flex-1 -mx-4 px-4 overflow-scroll no-scrollbar shadow-lg">
        <ul>
          ...
          {menus
            .filter((menu) => menu.section === 'setting')
            .map((menu) => {
                  ...
                  >
                    ...
                    {menu.children && (
                      <svg
                        ...
                        strokeWidth={1.5}
                        stroke="currentColor"
                        className={`w-4 h-4 ${menuOpen === menu.code ? 'rotate-90' : ''}`}
                      >
                        <path
                          strokeLinecap="round"
                          strokeLinejoin="round"
          ...
          {menus
            .filter((menu) => menu.section === 'master')
            .map((menu) => {
              if (!userPermissions.includes(menu.code)) {
                return null
              }
      ...
      <div className="m-auto p-4 text-center shadow-lg -mx-4">
        <p className="text-micro text-white">
          {`${dayjs().format('YYYY')} © ${process.env.REACT_APP_APARTMENT_NAME}`}
        </p>
        <p className="text-micro text-white">Powered by MetroSpace</p>
      </div>
    </aside>
  )
}
