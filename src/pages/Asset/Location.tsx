...
function PageAssetLocation() {
  ...
  const apiSubmitUpdate = () => api({
    url: `/v1/asset-location/${fields.id}`,
    withAuth: true,
    method: 'PUT',
    data: fields,
  })
  ...
  const tableDatas = data.data.map((column) => ({
    ...
    action: (
      <div className="flex items-center gap-1">
        ...
        {userPermissions.includes('asset-location-edit') && (
        ...
        {userPermissions.includes('asset-location-delete') && (
    ...
    <Layout>
      ...
      <Modal open={modalForm.open} title={modalForm.title}>
        <form autoComplete="off" className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-6" onSubmit={() => handleClickConfirm(fields.id ? 'update' : 'create')}>
          <Input
            placeholder="Nama Lokasi Aset"
            label="Nama Lokasi Aset"
            name="name"
            value={fields.name}
...
export default PageAssetLocation
