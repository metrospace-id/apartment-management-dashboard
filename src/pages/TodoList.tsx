  ...
  return (
    <Layout>
      ...
      <div className="p-4 dark:bg-slate-900 w-[100vw] sm:w-full">
        <div className="w-full flex p-4 bg-white rounded-lg dark:bg-black">
          <div className="w-[250px] pr-4 border-r-1 border-slate-200">
          ...
          <div className="flex-1 px-4 flex gap-2 flex-col">
            {isLoadingData && <LoadingContent />}
            {!isLoadingData && data.map((todo) => (
              <div className="p-2 border-1 rounded-lg flex gap-2 items-center" key={todo.id}>
                <Checkbox checked={todo.is_done} onClick={() => handleCheckTodo(todo.id)} />
                <p className={`text-sm text-slate-600 font-semibold ${todo.is_done ? 'line-through' : ''}`}>
                  {todo.title}
                </p>
              </div>
            ))}
